...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    total = 0
    sow_sad = False  # 使用更有意义的变量名

    for i in range(num_rolls):
        outcome = dice()
        if outcome == 1:
            sow_sad = True
        # 即使触发了 sow_sad，我们也可以继续加分（反正最后会丢弃），
        # 或者你可以写 else: total += outcome，这取决于个人喜好，不影响结果。
        total += outcome
    if sow_sad:
        return 1
    else:
...
def boar_brawl(player_score, opponent_score):
    ...
    score = max(abs(player-opponent)*3, 1)
    return score
    # END PROBLEM 2
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    score_zero=sus_update(0,score,opponent_score)
    if score_zero>=100:
        return 0
    return sus_strategy(score, opponent_score, threshold=9, num_rolls=6)
    # END PROBLEM 12
